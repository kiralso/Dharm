...
- (void)layoutSubviews {
    // Yay for math!
    CGFloat x = MAX(CGRectGetMidX(self.frame), self.superview.superview.superview.frame.size.width - CGRectGetMidX(self.frame));
    CGFloat y = MAX(CGRectGetMidY(self.frame), self.superview.superview.superview.frame.size.height - CGRectGetMidY(self.frame));
